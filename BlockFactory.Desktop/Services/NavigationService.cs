...
namespace BlockFactory.Desktop.Services
{
    public class NavigationService : INavigationService
    {
        ...
        /// <summary>
        /// معامل اختياري يُمرَّر مع NavigateTo قبل استدعائه.
        /// مثال: NavigationParameter = reservationId; NavigateTo("Withdrawal");
        /// </summary>
        ...
        private UserControl? ResolveView(string viewName) => viewName switch
        {
            ...
            // ─── الحجوزات (جديد) ────────────────────────────────────
            "Reservations" => _serviceProvider.GetRequiredService<ReservationsView>(),
            "NewReservation" => _serviceProvider.GetRequiredService<NewReservationView>(),
            "Withdrawal" => ResolveWithdrawal(),

            // ─── الإنتاج والمخزون ───────────────────────────────────
            "Production" => _serviceProvider.GetRequiredService<ProductionView>(),
            "Inventory" => _serviceProvider.GetRequiredService<InventoryView>(),

            // ─── الموردون والموارد البشرية ──────────────────────────
            "Suppliers" => _serviceProvider.GetRequiredService<SuppliersView>(),
            "Workers" => _serviceProvider.GetRequiredService<WorkersView>(),
            "Salaries" => _serviceProvider.GetRequiredService<SalariesView>(),
        ...
        /// <summary>
        /// شاشة السحب تحتاج تمرير ReservationId عبر NavigationParameter.
        /// مثال الاستخدام:
        ///   NavigationParameter = reservationId;
        ///   NavigateTo("Withdrawal");
        /// </summary>
        private WithdrawalView? ResolveWithdrawal()
        {
            var view = _serviceProvider.GetRequiredService<WithdrawalView>();
            if (NavigationParameter is int reservationId)
                view.LoadReservation(reservationId);
            return view;
        }
    }
}
